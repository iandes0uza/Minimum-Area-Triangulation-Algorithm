...
def buildTriangles( slice0, slice1 ):
    ...
    minDist = [distance(slice0.verts[0],slice1.verts[0]), slice0.verts[0],slice1.verts[0]] #Starting pair
    for v1 in slice0.verts:
        for v2 in slice1.verts:
            dist = distance(v1,v2)          #Distance between vertices
            if dist<minDist[0]:             #Checking if its a smaller distance
                minDist = [dist, v1,v2]
                
    #print("Shortest distance:", minDist[0], "  points: (",minDist[1],minDist[2],")")
    ...
    p0 = slice0.verts.index(minDist[1])  #Index of slice 0 closest point
    p1 = slice1.verts.index(minDist[2])  #Index of slice 1 closest point
    perm0 = slice0.verts[p0:] + slice0.verts[:p0] + [minDist[1]]
    perm1 = slice1.verts[p1:] + slice1.verts[:p1] + [minDist[2]]
#           after the index      before the index    the index
    ...
    minArea = [] 
    minDir  = [] 
    for row in range(len(perm1)):            #Just making 2, 2D lists
        minArea.append([])
        minDir.append([])
        for col in range(len(perm0)):
            minArea[row].append([])
            minDir[row].append([])
    ...
    minDir[0][0] = "."  # Little confused about Dir.PREV thing, I just used ".","-","|" which should serve the same purpose 

    # Fill in row 0 of minArea and minDir, since it's a special case as there's no row -1
    #
    # [2 marks]

    for col in range(1,len(perm0)):    #Don't need the first element, calculating first row
        minArea[0][col] = minArea[0][col-1] + triangleArea(perm1[0].coords,perm0[col].coords,perm0[col-1].coords) 
        minDir[0][col] = "-"
        #minDir[0][col] = Dir.PREV_ROW
        
    # Fill in col 0 of minArea and minDir, since it's a special case as there's no col -1
    #
    # [2 marks]
    for row in range(1,len(perm1)):    #Don't need first element, calculating first col
        minArea[row][0] = minArea[row-1][0] + triangleArea(perm0[0].coords,perm1[row].coords,perm1[row-1].coords)  
        minDir[row][0] = "|"
        #minDir[row][0] = Dir.PREV_COL
    ...
    for row in range(1,len(perm1)):
        for col in range(1,len(perm0)):
            A1 = minArea[row-1][col] + triangleArea(perm0[col].coords, perm1[row].coords, perm1[row-1].coords)    #Calculating area's 
            A2 = minArea[row][col-1] + triangleArea(perm1[row].coords, perm0[col].coords, perm0[col-1].coords)
            #A1 = minArea[row-1][col] + triangleArea(perm1[col].coords, perm0[row].coords, perm0[row-1].coords)
            #A2 = minArea[row][col-1] + triangleArea(perm0[row].coords, perm1[col].coords, perm1[col-1].coords)
            if A1<A2:                      #Checking if smallest area came from A1
                minArea[row][col] = A1
                minDir[row][col] = "|"
            elif A2<=A1:                   #Or A2
                minArea[row][col] = A2
...
##    print('\t  ',end = "")
##    for col in range(len(minArea[0])):
##        print(col,end = "\t  ")
##    print()
##    for row in range(len(minArea)):
##        print(row,end = "\t")
##        for col in range(len(minArea[row])):
##            print(minDir[row][col],round(minArea[row][col]),end = "\t")
##        print()
    ...
    triangles = []
    x = len(perm0)-1        #Domain
    y = len(perm1)-1        #Range
    while (x,y)!=(0,0):     #Not at "."
        if minDir[y][x] == "|":   #If we have a "|", traverse up
            triangles.append(Triangle([perm0[x],perm1[y],perm1[y-1]]))
            y-=1
        elif minDir[y][x] == "-": #If we have a "-", traverse left
            triangles.append(Triangle([perm0[x],perm1[y],perm0[x-1]]))
            x-=1
            
##        if minDir[y][x] == Dir.PREV_COL:   #If we have a "|", traverse up
##            triangles.append(Triangle([perm0[x],perm1[y],perm1[y-1]]))
##            y-=1
##        elif minDir[y][x] == Dir.PREV_ROW: #If we have a "-", traverse left
##            triangles.append(Triangle([perm0[x],perm1[y],perm0[x-1]]))
##            x-=1
